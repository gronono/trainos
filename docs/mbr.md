...
The MBR is placed in the first sector of the disk, making it 512 bytes in size.
...
The code executed by the BIOS is called the bootloader. Its main task is to search for an active partition and boot the operating system contained within that partition.

### MBR layout

Because there are many of optional values, the following table only shown by the `parted` or `fdisk` tools.
...
Each primary partition entry has the following format: (offset is relative to the start of the partition entry)
...
The LBA of the first partition's start is `0x00000800`. This corresponds to the expected value of 2048 sectors that you passed to `parted` when creating the partition.

The number of sectors in the first partition is `0x00005000`. Since the sector size is 512 bytes, the partition has the expected size of 10 MiB.
