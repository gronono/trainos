...
SECTOR_SIZE=512
STAGE2_SIZE=$(stat /trainos/build/boot/legacy/stage2.bin -c %s)
...
printf "%b" "$hexa" >> /trainos/build/boot/legacy/stage1.bin
...
info "Copy Stage 1"
try dd if=/trainos/build/boot/legacy/stage1.bin of="${DISK}" bs=1 seek=0 conv=notrunc

info "Copy Stage 2"
try dd if=/trainos/build/boot/legacy/stage2.bin of="${DISK}" bs=512 seek=1 conv=notrunc
